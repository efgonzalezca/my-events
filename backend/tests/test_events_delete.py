...
def test_owner_deletes_draft_and_subsequent_get_returns_404(
    ...
    after = client.get(f"/api/events/{event_id}")
    assert after.status_code == 404
    assert after.json()["code"] == "EVENT_NOT_FOUND"


def test_delete_published_returns_409_not_modifiable(
    client: TestClient, engine
) -> None:
...
def test_delete_not_owner_is_forbidden(client: TestClient, engine) -> None:
    _, owner_id = _register_login_promote(
        client, engine, "owner@test.com", UserRole.organizer
    )
...
def test_delete_nonexistent_returns_404(client: TestClient, engine) -> None:
